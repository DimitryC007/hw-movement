...
| `GET` | `/api/v1/data/{id}` | Get a data item by ID |
...
- **Domain** — core entity (`DataItem`), no dependencies
- **Application** — business logic (`DataService`), interfaces (`IDataRepository`, `IDataService`)
- **Infrastructure** — implementations (PostgreSQL, Redis, custom cache)
- **WebApi** — controllers, middleware, DI composition root
- **WebHost** — entry point, hosting configuration

---

### Repository Pattern

`IDataRepository` abstracts data access from the application layer. The concrete implementation (`DataRepository`) uses Entity Framework Core against PostgreSQL.

---

### Decorator Pattern — Cached Repository

`CachedDataRepository` wraps `DataRepository` and adds a **two-tier cache-aside strategy** transparently, without changing the application layer.
...
DataService
    └── CachedDataRepository  (decorator)
            ...
            └── DataRepository (PostgreSQL)            ← fallback
```
...
```
├── Domain/                         # Core entity (DataItem)
├── Application/                    # Business logic, interfaces, validators
│   ├── Data/                       # IDataService, IDataRepository, DataService, DataValidator
...
│   └── Persistence/
│       └── Data/                   # DataRepository + CachedDataRepository
